...
class TestViewsPSU(SimpleTestCase):
    ...
    def setUp(self):
        ...
        # Mock de TODOS los context processors que hacen queries
        self.context_processor_patcher1 = patch('universitaryWellbeing.context_processors.user_role', 
                                                return_value={'user_role': None})
        self.context_processor_patcher1.start()
        
        self.context_processor_patcher2 = patch('universitaryWellbeing.context_processors.notificaciones_context', 
                                                return_value={'notificaciones': [], 'notificaciones_no_leidas': 0})
        self.context_processor_patcher2.start()
    
    def tearDown(self):
        """Limpiar patches"""
        self.login_patcher.stop()
        self.context_processor_patcher1.stop()
        self.context_processor_patcher2.stop()
    ...
    def test_crear_proyecto_admin_ok(self, mock_parse_date, mock_is_admin, mock_current_part, 
                                      mock_redirect, mock_messages, mock_proyectos):
